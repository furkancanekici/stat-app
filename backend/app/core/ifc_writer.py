...
def write_enriched_ifc(elements: list[dict], connectivity: dict) -> bytes:
    ...
    for i, sname in enumerate(stories):
        ...
        add(f"#{cp}=IFCCARTESIANPOINT((0.,0.,{elev}));")
        add(f"#{ap}=IFCAXIS2PLACEMENT3D(#{cp},#7,#6);")
        add(f"#{st}=IFCBUILDINGSTOREY('{guid}',#5,'{sname}',$,$,#{ap},$,$,.ELEMENT.,{elev}.);")
        story_ids[sname] = st
    ...
    for el in elements:
        ...
        color = STATUS_COLORS.get(status, STATUS_COLORS["UNMATCHED"])
        ...
        add(f"#{p2e}=IFCCARTESIANPOINT(({length}.,0.,0.));")
        add(f"#{seg}=IFCPOLYLINE((#{p1e},#{p2e}));")
        add(f"#{ax_rep}=IFCSHAPEREPRESENTATION(#10,'Axis','Curve3D',(#{seg}));")

        if el_type == "IfcColumn":
            add(f"#{ext}=IFCEXTRUDEDAREASOLID(#9,#9,#7,{length}.);")
        else:
            add(f"#{ext}=IFCEXTRUDEDAREASOLID(#9,#9,#6,{length}.);")
        ...
        add(f"#{lp_cp}=IFCCARTESIANPOINT(({x1},{y1},{z1}));")
        dir1 = eid; eid+=1
        dir2 = eid; eid+=1
        add(f"#{dir1}=IFCDIRECTION((0.,0.,1.));")
        add(f"#{dir2}=IFCDIRECTION(({dx},{dy},{dz}));")
